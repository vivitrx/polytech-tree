...
- 点击节点：高亮它的关系——基石（沿前置递归 3 层，青色）与后继（以它为基石的节点，橙色），其余节点压暗、名字隐藏；带箭头的连线指示方向；信息卡列出基石/后继名可直接点击跳转；屏幕下方出现面包屑（如 计算复杂性理论 → 哥德尔不完备定理），点击可跳回；再点空白恢复
- `Ctrl+F`：打开搜索框，按中英文名与别名匹配；回车或点选后相机平滑推近，把节点置于屏幕中心并高亮
- 空格：切换冻结/恢复（节点自转、相机自动旋转、漫游同时暂停）；冻结时信息卡固定且可点，便于打开卡里的英文维基条目链接
